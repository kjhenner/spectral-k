...
from spectral_k.mel_helpers import image_to_audio
...
def main():
    ...
    size = (
        ...
        if len(model_config["input_size"]) == 2
        else [model_config["input_size"], model_config["input_size"]]
    )

    ddp_kwargs = accelerate.DistributedDataParallelKwargs(
        find_unused_parameters=model_config["skip_stages"] > 0
    )
    ...
    if dataset_config["type"] == "imagefolder":
        ...
        train_set = datasets.CIFAR10(
            dataset_config["location"], train=True, download=True, transform=tf
        )
        ...
        train_set.set_transform(
            partial(
                K.utils.hf_datasets_augs_helper,
                transform=tf,
    ...
    if state_path.exists() or args.resume:
        ...
        accelerator.unwrap_model(model_ema.inner_model).load_state_dict(
            ckpt["model_ema"]
        )
    ...
    if evaluate_enabled:
        ...
        reals_features = K.evaluation.compute_features(
            accelerator,
            lambda x: next(train_iter)[image_key][1],
            extractor,
            args.evaluate_n,
            args.batch_size,
        )
        if accelerator.is_main_process:
            metrics_log = K.utils.CSVLogger(
                f"{args.name}_metrics.csv", ["step", "fid", "kid"]
            )
        del train_iter

    @torch.no_grad()
    @K.utils.eval_mode(model_ema)
    def demo(convert_spectrogram=True):
        if accelerator.is_main_process:
            tqdm.write("Sampling...")
        ...
        x = (
            torch.randn(
                [n_per_proc, model_config["input_channels"], size[0], size[1]],
                device=device,
            )
            * sigma_max
        )
        sigmas = K.sampling.get_sigmas_karras(
            50, sigma_min, sigma_max, rho=7.0, device=device
        )
        x_0 = K.sampling.sample_lms(
            model_ema, x, sigmas, disable=not accelerator.is_main_process
        )
        x_0 = accelerator.gather(x_0)[: args.sample_n]
        if accelerator.is_main_process:
            if convert_spectrogram and step > 100:
                for i, example in enumerate(x_0):
                    image = K.utils.to_pil_image(example)
                    image.save(f"{i}_sample.png")
                    audio = image_to_audio(image)
                    if use_wandb:
                        wandb.log(
                            {f"{i}_audio": wandb.Audio(audio, sample_rate=22050)},
                            step=step,
                        )
    ...
    def evaluate():
        ...
        def sample_fn(n):
            x = (
                torch.randn(
                    [n, model_config["input_channels"], size[0], size[1]], device=device
                )
                * sigma_max
            )
        ...
        fakes_features = K.evaluation.compute_features(
            accelerator, sample_fn, extractor, args.evaluate_n, args.batch_size
        )
        if accelerator.is_main_process:
                ...
                if accelerator.is_main_process:
                    if step % 25 == 0:
                        if args.gns:
                            ...
                            tqdm.write(
                                f"Epoch: {epoch}, step: {step}, loss: {loss.item():g}"
                            )
